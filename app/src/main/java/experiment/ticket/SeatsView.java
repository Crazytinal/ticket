...
public class SeatsView extends View {
    ...
    public SeatsView(Context context, AttributeSet attrs) {
        ...
        // 中心往上
        screenCenter_y = windowHeight / 2 - 600;
    ...
    private void drawJUXING(Canvas canvas, float x, float y, float width, float heigh, boolean isGreen) {
        Path path = new Path();
        path.moveTo(x, y);
        ...
        for (int i = 0; i < seats.length; i++) {
            for (int j = 0; j < seats[0].length; j++) {
                ...
                if (isOuShu) {
                    ...
                    leftTop_y = screenCenter_y + heigh * i;
                    if (leftTop_x > x_num) {
                        drawJUXING(canvas, leftTop_x, leftTop_y, width, heigh, true);
                    } else {
                        drawJUXING(canvas, leftTop_x, leftTop_y, width, heigh, false);
                    }
    ...
    public Point screenToindex(float t_x, float t_y) {
        ...
        for (int i = 0; i < pointXYList.size(); i++) {
            if (t_x > pointXYList.get(i)[0]
                    ...
                    && t_y > pointXYList.get(i)[1]+9*heigh
                    && t_y < (pointXYList.get(i)[1] + 10*heigh)) {
                point = new Point();
